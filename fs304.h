...
/****************************************************************************/
/* checks whether file1 & file2 exists in current directory
 /* display error if not exist
 /* if so, compare their content to current ones and display a message saying;
 /** "files have 0 difference" if identical
 /** "%s many characters differ" if different
 /*
 /****************************************************************************/
...
void compare(char *f1, char *f2)
{
    ...
    int i,j,k;
    int e_inode, e_inode1, e_inode2; // info inodes
    
    char file1[1024], file2[1024]; // file content buffers
    int blockNumber; // block number to read content of files
    
    if (strcmp(f1,f2) == 0)
    {
    	printf("Who would want to compare something to itself?\n");
    	return;
    }
    ...
    // adds file entries to f_names array
    for (i=0; i<3; i++) {
        if (blocks[i]==0) continue;	// 0 means pointing at nothing
        ...
        for (j=0; j<4; j++) {
            ...
            else
            {
            	if (strcmp(_directory_entries[j].fname, f1) == 0 ||
            		strcmp(_directory_entries[j].fname, f2) == 0)
            	{
            		printf("Can not compare a DIR\n");
            		return;
            	}
            }
        }
...
//        printf("%s has been found in block: %d, buffer: %d\n%s has been found in block: %d, buffer: %d\n", f1, blk1, bfr1, f2, blk2, bfr2);
        ...
        e_inode2 = stoi(_directory_entries[bfr2].MMM,3); // THIS IS OUR FILE2
        
        /* Read file1's content */
        if(_inode_table[e_inode1].XX[0]!='0' || _inode_table[e_inode1].XX[1]!='0'){
            // There is data on this block. Let's read it.
            blockNumber = 0;
            for(i=0; i<2; i++){
                blockNumber = blockNumber * 10 + (_inode_table[e_inode1].XX[i] - '0');
            }
        ...
        if(_inode_table[e_inode1].YY[0]!='0' || _inode_table[e_inode1].YY[1]!='0'){
            ...
            blockNumber = 0;
            for(i=0; i<2; i++){
                blockNumber = blockNumber * 10 + (_inode_table[e_inode1].YY[i] - '0');
            }
            readFS304(blockNumber, file1);
        }
		/*Read file2's content */
        ...
        if(_inode_table[e_inode1].ZZ[0]!='0' || _inode_table[e_inode1].ZZ[1]!='0') {
            ...
            blockNumber = 0;
            for(i=0; i<2; i++){
                blockNumber = blockNumber * 10 + (_inode_table[e_inode1].ZZ[i] - '0');
            }
            readFS304(blockNumber, file1);
        }
        /* Read file2's content */
        if(_inode_table[e_inode2].ZZ[0]!='0' || _inode_table[e_inode2].ZZ[1]!='0') {
            ...
            blockNumber = 0;
            for(i=0; i<2; i++){
                blockNumber = blockNumber * 10 + (_inode_table[e_inode2].ZZ[i] - '0');
            }
            readFS304(blockNumber, file2);
        }
		...
		{
        	printf("%d many characters are different in %s and %s.\n", result, f1, f2);
		}
...
//				    printf("SIZE: %d\n", toCopy.size);
				    ...
				    bool enoughSpace = requestBlocks(toCopy.size,newBlockIndices);
				    ...
				    {
				    	_inode_table[newInode].TT[0] = 'F';
				    	for (int k = 0; k < toCopy.size; k++)
				    	{
				    		writeFS304(newBlockIndices[k], toCopy.blocks[k]);
				    	}
				    	itos(_inode_table[newInode].XX, newBlockIndices[0], 2);
